...
/*=============================================================================
  ...
  ===========================================================================*/
// Define the enum for qshGeofenceStatus
typedef enum {
    SERVICE_UNKNOWN = 0,
    SERVICE_AVAILABLE = 1,
...
static qshGeofenceStatus geoServiceStatus = SERVICE_UNKNOWN;
...
// Function to print the menu based on geoServiceStatus
static void printMenu() {
    switch (geoServiceStatus) {
        ...
        case SERVICE_UNAVAILABLE:
            printf("\n\n"
                   "qsh geofence sensor is unavailable!\n"
                   "q: quit\n");
            break;
        default:
            break;
...
static const char* event_cb_msg_string(uint32_t message_id) {
    ...
    case 512:
        return "QSH_GEOFENCE_MSGID_QSH_GEOFENCE_OPEN";
    case 513:
        return "QSH_GEOFENCE_MSGID_QSH_GEOFENCE_CLOSE";
    case 514:
        return "QSH_GEOFENCE_MSGID_QSH_GEOFENCE_UPDATE";
    case 768:
        return "QSH_GEOFENCE_MSGID_QSH_GEOFENCE_ACK";
    case 1024:
        return "QSH_GEOFENCE_MSGID_QSH_GEOFENCE_BREACH_EVENT";
    default:
    ...
    for (int i = 0; i < pb_event_msg.events_size(); i++) {
        ...
        if (SNS_STD_MSGID_SNS_STD_ERROR_EVENT == pb_event.msg_id()) {
            ...
            for (int i = 0; i < idsSize; i++) {
                for (int j=0; j< GNSS_MAX_GEOFENCES; ++j) {
                    if (myGeofences[j].id == breach.ids(i)) {
                        printf("Geofence ids[%d]=%d\n", i, j);
                        LOC_LOGd("Geofence ids[%d]=%d", i, j);
                        break;
                    }
                }
            }
            ...
            LOC_LOGd("Unsupprted message ID");
        }
    }
}
...
static void suid_cb(const std::string& datatype, const std::vector<suid>& suids) {
    ...
    qshGeofenceStatus prevStatus = geoServiceStatus;

    if (suids.size() > 0) {
        mySuid = suids[0];
        // create a new ISession which listens to the event_cb
        ...
        if (0 == ret) {
            LOC_LOGd("requesting attributes for - suid_low=%" PRIu64 " suid_high=%" PRIu64 "\n",
                    mySuid.low, mySuid.high);
            lookup.getAttributes(myIsession, mySuid);
            geoServiceStatus = SERVICE_AVAILABLE;
        } else {
            LOC_LOGe("set event CallBacks failed");
            geoServiceStatus = SERVICE_UNAVAILABLE;
        }
    } else {
        LOC_LOGe("%s sensor is not available", SENSOR_NAME);
        geoServiceStatus = SERVICE_UNAVAILABLE;
    }
    if (prevStatus != geoServiceStatus && prevStatus != SERVICE_UNKNOWN) {
        if (SERVICE_UNAVAILABLE == geoServiceStatus) {
            printf("\n%s sensor is not available", SENSOR_NAME);
            loadGeofences();
            isInitialized = false;
        }
        if (SERVICE_AVAILABLE == geoServiceStatus) {
...
int test_gf_start(uint32_t request, uint32_t count) {
    ...
    if (geoServiceStatus != SERVICE_AVAILABLE) {
        LOC_LOGd("No action for geoServiceStatus %d", geoServiceStatus);
        return 0;
    }
    ...
    while (0 == exit_loop) {
        ...
        if (geoServiceStatus == SERVICE_AVAILABLE) {
            ...
            case '8':
                ...
                ret = test_gf_start(QSH_GEOFENCE_RESUME, newCount);
                if (0 == ret) {
                    printf("success\n");
                } else {
                    printf("failed\n");
